...
return escapedTxt;}
Inputmask.extendAliases({"numeric":{mask:function(opts){if(opts.repeat!==0&&isNaN(opts.integerDigits)){opts.integerDigits=opts.repeat;}
opts.repeat=0;if(opts.groupSeparator===opts.radixPoint){if(opts.radixPoint==="."){opts.groupSeparator=",";}else if(opts.radixPoint===","){opts.groupSeparator=".";}else opts.groupSeparator="";}
...
var mask="[+]";mask+=autoEscape(opts.prefix,opts);if(opts.integerOptional===true){mask+="~{1,"+opts.integerDigits+"}";}else mask+="~{"+opts.integerDigits+"}";if(opts.digits!==undefined){opts.radixPointDefinitionSymbol=opts.decimalProtect?":":opts.radixPoint;var dq=opts.digits.toString().split(",");if(isFinite(dq[0]&&dq[1]&&isFinite(dq[1]))){mask+=opts.radixPointDefinitionSymbol+";{"+opts.digits+"}";}else if(isNaN(opts.digits)||parseInt(opts.digits)>0){if(opts.digitsOptional){mask+="["+opts.radixPointDefinitionSymbol+";{1,"+opts.digits+"}]";}else mask+=opts.radixPointDefinitionSymbol+";{"+opts.digits+"}";}}
mask+=autoEscape(opts.suffix,opts);mask+="[-]";opts.greedy=false;return mask;},placeholder:"",greedy:false,digits:"*",digitsOptional:true,radixPoint:".",positionCaretOnClick:"radixFocus",groupSize:3,groupSeparator:"",autoGroup:false,allowMinus:true,negationSymbol:{front:"-",back:""},integerDigits:"+",integerOptional:true,prefix:"",suffix:"",rightAlign:true,decimalProtect:true,min:null,max:null,step:1,insertMode:true,autoUnmask:false,unmaskAsNumber:false,inputmode:"numeric",preValidation:function(buffer,pos,c,isSelection,opts){if(c==="-"||c==opts.negationSymbol.front){if(opts.allowMinus!==true)return false;opts.isNegative=opts.isNegative===undefined?true:!opts.isNegative;if(buffer.join("")==="")return true;return{caret:pos,dopost:true};}
if(isSelection===false&&c===opts.radixPoint&&(opts.digits!==undefined&&(isNaN(opts.digits)||parseInt(opts.digits)>0))){var radixPos=$.inArray(opts.radixPoint,buffer);if(radixPos!==-1){if(opts.numericInput===true){return pos===radixPos;}
return{"caret":radixPos+1};}}
return true;},postValidation:function(buffer,currentResult,opts){function buildPostMask(buffer,opts){var postMask="";postMask+="("+opts.groupSeparator+"*{"+opts.groupSize+"}){*}";if(opts.radixPoint!==""){var radixSplit=buffer.join("").split(opts.radixPoint);if(radixSplit[1]){postMask+=opts.radixPoint+"*{"+radixSplit[1].match(/^\d*\??\d*/)[0].length+"}";}}
return postMask;}
...
if(processValue.length>1&&processValue.indexOf(opts.radixPoint)!==1){if(charAtPos=="0"){processValue=processValue.replace(/^\?/g,"");}
processValue=processValue.replace(/^0/g,"");}
if(processValue.charAt(0)===opts.radixPoint&&opts.numericInput!==true){processValue="0"+processValue;}
if(processValue!==""){processValue=processValue.split("");if(!opts.digitsOptional&&isFinite(opts.digits)){var radixPosition=$.inArray(opts.radixPoint,processValue);var rpb=$.inArray(opts.radixPoint,maskedValue);if(radixPosition===-1){processValue.push(opts.radixPoint);radixPosition=processValue.length-1;}
for(var i=1;i<=opts.digits;i++){if(!opts.digitsOptional&&(processValue[radixPosition+i]===undefined||processValue[radixPosition+i]===opts.placeholder.charAt(0))){processValue[radixPosition+i]=currentResult.placeholder||opts.placeholder.charAt(0);}else if(rpb!==-1&&maskedValue[rpb+i]!==undefined){processValue[radixPosition+i]=processValue[radixPosition+i]||maskedValue[rpb+i];}}}
if(opts.autoGroup===true&&opts.groupSeparator!==""&&(charAtPos!==opts.radixPoint||currentResult.pos!==undefined||currentResult.dopost)){processValue=Inputmask(buildPostMask(processValue,opts),{numericInput:true,jitMasking:true,definitions:{"*":{validator:"[0-9?]",cardinality:1}}}).format(processValue.join(""));if(processValue.charAt(0)===opts.groupSeparator){processValue.substr(1);}}else processValue=processValue.join("");}
...
processValue=processValue.split("");if(charAtPos!==undefined){if(charAtPos!==opts.radixPoint&&charAtPos!==opts.negationSymbol.front&&charAtPos!==opts.negationSymbol.back){caretPos=$.inArray("?",processValue);if(caretPos>-1)
processValue[caretPos]=charAtPos;else caretPos=currentResult.caret||0;}else if(charAtPos===opts.radixPoint||charAtPos===opts.negationSymbol.front||charAtPos===opts.negationSymbol.back){var newCaretPos=$.inArray(charAtPos,processValue);if(newCaretPos!==-1)caretPos=newCaretPos;}}
if(opts.numericInput){caretPos=processValue.length-caretPos-1;processValue=processValue.reverse();}
var rslt={caret:(charAtPos===undefined||currentResult.pos!==undefined)?caretPos+(opts.numericInput?-1:1):caretPos,buffer:processValue,refreshFromBuffer:currentResult.dopost||buffer.join("")!==processValue.join("")};return rslt.refreshFromBuffer?rslt:currentResult;},onBeforeWrite:function(e,buffer,caretPos,opts){function parseMinMaxOptions(opts){if(opts.parseMinMaxOptions===undefined){if(opts.min!==null){opts.min=opts.min.toString().replace(new RegExp(Inputmask.escapeRegex(opts.groupSeparator),"g"),"");if(opts.radixPoint===",")opts.min=opts.min.replace(opts.radixPoint,".");opts.min=isFinite(opts.min)?parseFloat(opts.min):NaN;if(isNaN(opts.min))opts.min=Number.MIN_VALUE;}
...
if(e){switch(e.type){case"keydown":return opts.postValidation(buffer,{caret:caretPos,dopost:true},opts);case"blur":case"checkval":var unmasked;parseMinMaxOptions(opts);if(opts.min!==null||opts.max!==null){unmasked=opts.onUnMask(buffer.join(""),undefined,$.extend({},opts,{unmaskAsNumber:true}));if(opts.min!==null&&unmasked<opts.min){opts.isNegative=opts.min<0;return opts.postValidation(opts.min.toString().replace(".",opts.radixPoint).split(""),{caret:caretPos,dopost:true,placeholder:"0"},opts);}else if(opts.max!==null&&unmasked>opts.max){opts.isNegative=opts.max<0;return opts.postValidation(opts.max.toString().replace(".",opts.radixPoint).split(""),{caret:caretPos,dopost:true,placeholder:"0"},opts);}}
return opts.postValidation(buffer,{caret:caretPos,dopost:true,placeholder:"0",event:"blur"},opts);case"_checkval":return{caret:caretPos};default:break;}}},regex:{integerPart:function(opts,emptyCheck){return emptyCheck?new RegExp("["+Inputmask.escapeRegex(opts.negationSymbol.front)+"\+]?"):new RegExp("["+Inputmask.escapeRegex(opts.negationSymbol.front)+"\+]?\\d+");},integerNPart:function(opts){return new RegExp("[\\d"+Inputmask.escapeRegex(opts.groupSeparator)+Inputmask.escapeRegex(opts.placeholder.charAt(0))+"]+");}},definitions:{"~":{validator:function(chrs,maskset,pos,strict,opts,isSelection){var isValid=strict?new RegExp("[0-9"+Inputmask.escapeRegex(opts.groupSeparator)+"]").test(chrs):new RegExp("[0-9]").test(chrs);if(isValid===true){if(opts.numericInput!==true&&maskset.validPositions[pos]!==undefined&&maskset.validPositions[pos].match.def==="~"&&!isSelection){var processValue=maskset.buffer.join("");processValue=processValue.replace(new RegExp("[-"+Inputmask.escapeRegex(opts.negationSymbol.front)+"]","g"),"");processValue=processValue.replace(new RegExp(Inputmask.escapeRegex(opts.negationSymbol.back)+"$"),"");processValue=processValue.replace(/0/g,opts.placeholder.charAt(0));var bufferTemplate=maskset._buffer.join("");if(processValue===opts.radixPoint){processValue=bufferTemplate;}
while(processValue.match(Inputmask.escapeRegex(bufferTemplate)+"$")===null){bufferTemplate=bufferTemplate.slice(1);}
processValue=processValue.replace(bufferTemplate,"");processValue=processValue.split("");if(processValue[pos]===undefined){isValid={"pos":pos,"remove":pos};}else{isValid={pos:pos};}}}else if(!strict&&chrs===opts.radixPoint&&maskset.validPositions[pos-1]===undefined){maskset.buffer[pos]="0";isValid={"pos":pos+1}}
...
return isValid;},cardinality:1,placeholder:function(opts){return opts.radixPoint;}}},onUnMask:function(maskedValue,unmaskedValue,opts){if(unmaskedValue===""&&opts.nullable===true){return unmaskedValue;}
...
processValue=processValue.replace(new RegExp(opts.placeholder.charAt(0),"g"),"0");if(opts.unmaskAsNumber){if(opts.radixPoint!==""&&processValue.indexOf(opts.radixPoint)!==-1)processValue=processValue.replace(Inputmask.escapeRegex.call(this,opts.radixPoint),".");return Number(processValue);}
return processValue;},isComplete:function(buffer,opts){var maskedValue=buffer.join(""),bufClone=buffer.slice();if(bufClone.join("")!==maskedValue)return false;var processValue=maskedValue.replace(opts.prefix,"");processValue=processValue.replace(opts.suffix,"");processValue=processValue.replace(new RegExp(Inputmask.escapeRegex(opts.groupSeparator),"g"),"");if(opts.radixPoint===",")processValue=processValue.replace(Inputmask.escapeRegex(opts.radixPoint),".");return isFinite(processValue);},onBeforeMask:function(initialValue,opts){opts.isNegative=undefined;initialValue=initialValue.toString().charAt(initialValue.length-1)===opts.radixPoint?initialValue.toString().substr(0,initialValue.length-1):initialValue.toString();if(opts.radixPoint!==""&&isFinite(initialValue)){var vs=initialValue.split("."),groupSize=opts.groupSeparator!==""?parseInt(opts.groupSize):0;if(vs.length===2&&(vs[0].length>groupSize||vs[1].length>groupSize||(vs[0].length<=groupSize&&vs[1].length<groupSize)))
initialValue=initialValue.replace(".",opts.radixPoint);}
var kommaMatches=initialValue.match(/,/g);var dotMatches=initialValue.match(/\./g);if(dotMatches&&kommaMatches){if(dotMatches.length>kommaMatches.length){initialValue=initialValue.replace(/\./g,"");initialValue=initialValue.replace(",",opts.radixPoint);}else if(kommaMatches.length>dotMatches.length){initialValue=initialValue.replace(/,/g,"");initialValue=initialValue.replace(".",opts.radixPoint);}else{initialValue=initialValue.indexOf(".")<initialValue.indexOf(",")?initialValue.replace(/\./g,""):initialValue=initialValue.replace(/,/g,"");}}else{initialValue=initialValue.replace(new RegExp(Inputmask.escapeRegex(opts.groupSeparator),"g"),"");}
if(opts.digits===0){if(initialValue.indexOf(".")!==-1){initialValue=initialValue.substring(0,initialValue.indexOf("."));}else if(initialValue.indexOf(",")!==-1){initialValue=initialValue.substring(0,initialValue.indexOf(","));}}
if(opts.radixPoint!==""&&isFinite(opts.digits)&&initialValue.indexOf(opts.radixPoint)!==-1){var valueParts=initialValue.split(opts.radixPoint),decPart=valueParts[1].match(new RegExp("\\d*"))[0];if(parseInt(opts.digits)<decPart.toString().length){var digitsFactor=Math.pow(10,parseInt(opts.digits));initialValue=initialValue.replace(Inputmask.escapeRegex(opts.radixPoint),".");initialValue=Math.round(parseFloat(initialValue)*digitsFactor)/digitsFactor;initialValue=initialValue.toString().replace(".",opts.radixPoint);}}
return initialValue;},canClearPosition:function(maskset,position,lvp,strict,opts){var vp=maskset.validPositions[position],canClear=vp.input!==opts.radixPoint||(maskset.validPositions[position].match.fn!==null&&opts.decimalProtect===false)||(vp.input===opts.radixPoint&&maskset.validPositions[position+1]&&maskset.validPositions[position+1].match.fn===null)||isFinite(vp.input)||position===lvp||vp.input===opts.groupSeparator||vp.input===opts.negationSymbol.front||vp.input===opts.negationSymbol.back;if(canClear&&(vp.match.nativeDef=="+"||vp.match.nativeDef=="-")){opts.isNegative=false;}
return canClear;},onKeyDown:function(e,buffer,caretPos,opts){var $input=$(this);if(e.ctrlKey){switch(e.keyCode){case Inputmask.keyCode.UP:$input.val(parseFloat(this.inputmask.unmaskedvalue())+parseInt(opts.step));$input.trigger("setvalue");break;case Inputmask.keyCode.DOWN:$input.val(parseFloat(this.inputmask.unmaskedvalue())-parseInt(opts.step));$input.trigger("setvalue");break;}}}},"currency":{prefix:"$ ",groupSeparator:",",alias:"numeric",placeholder:"0",autoGroup:true,digits:2,digitsOptional:false,clearMaskOnLostFocus:false},"decimal":{alias:"numeric"},"integer":{alias:"numeric",digits:0,radixPoint:""},"percentage":{alias:"numeric",digits:2,digitsOptional:true,radixPoint:".",placeholder:"0",autoGroup:false,min:0,max:100,suffix:" %",allowMinus:false}});return Inputmask;}));var _=function(obj){if(obj instanceof _)return obj;if(!(this instanceof _))return new _(obj);this._wrapped=obj;};_.now=Date.now||function(){return new Date().getTime();};var hasEnumBug=!{toString:null}.propertyIsEnumerable('toString');var nonEnumerableProps=['valueOf','isPrototypeOf','toString','propertyIsEnumerable','hasOwnProperty','toLocaleString'];var collectNonEnumProps=function(obj,keys){var nonEnumIdx=nonEnumerableProps.length;var constructor=obj.constructor;var proto=constructor.prototype||ObjProto;var prop='constructor';if(has(obj,prop)&&!_.contains(keys,prop))keys.push(prop);while(nonEnumIdx--){prop=nonEnumerableProps[nonEnumIdx];if(prop in obj&&obj[prop]!==proto[prop]&&!_.contains(keys,prop)){keys.push(prop);}}};_.isObject=function(obj){var type=typeof obj;return type==='function'||type==='object'&&!!obj;};var createAssigner=function(keysFunc,defaults){return function(obj){var length=arguments.length;if(defaults)obj=Object(obj);if(length<2||obj==null)return obj;for(var index=1;index<length;index++){var source=arguments[index],keys=keysFunc(source),l=keys.length;for(var i=0;i<l;i++){var key=keys[i];if(!defaults||obj[key]===void 0)obj[key]=source[key];}}
...
return result;};throttled.cancel=function(){clearTimeout(timeout);previous=0;timeout=context=args=null;};return throttled;};var bind=function(fn,me){return function(){return fn.apply(me,arguments);};};RangeSliderField=(function(){RangeSliderField.prototype.DEFAULT_TYPE='default';RangeSliderField.prototype.KEY_CODES={UP:38,DOWN:40};function RangeSliderField(options){if(options==null){options={};}
this._onInputBlur=bind(this._onInputBlur,this);this._onInputKeyDown=bind(this._onInputKeyDown,this);this._onInputFocus=bind(this._onInputFocus,this);this._onInputUpdate=bind(this._onInputUpdate,this);if(!options.DOMElement){throw new Error("ERROR :: RangeSliderField class :: 'DOMElement' options is required");}
if($(options.DOMElement).data('component')!=null){return;}
this.$rangeSlider=$(options.DOMElement).find('.range__slider');_.defaults(options,{type:this.DEFAULT_TYPE});this._init(options);this._addEventListeners();$(options.DOMElement).data('component',this);}
RangeSliderField.prototype._init=function(options){if(options==null){options={};}
this._initUI();this._initData(options);this.parseFunction=this.data.type==='apr'?parseFloat:parseInt;this._initRangeSlider();return this._initRangeInput();};RangeSliderField.prototype._initUI=function(){this.ui={};this.ui.$rangeBlock=this.$rangeSlider.closest('.range');return this.ui.$rangeInput=this.ui.$rangeBlock.find('.range__input');};RangeSliderField.prototype._initData=function(options){this.data={};this.data.type=options.type;this.data.defaultValue=this.$rangeSlider.data('from');this.data.maxValue=this.$rangeSlider.data('max');this.data.minValue=this.$rangeSlider.data('min');this.data.step=this.$rangeSlider.data('step');return this.callbacks={};};RangeSliderField.prototype._initRangeSlider=function(){this.$rangeSlider.attr('tabindex',-1);this.$rangeSlider.ionRangeSlider({hide_min_max:true,hide_from_to:true,onChange:(function(_this){return function(data){_this._setValue(data.from);return _this._dirtySlider();};})(this),onStart:(function(_this){return function(data){return _this._setValue(_this.data.defaultValue);};})(this),onFinish:(function(_this){return function(){_this.ui.$rangeInput.blur();return _this._unHighlightSlider();};})(this)});return this.$rangeSliderInstance=this.$rangeSlider.data('ionRangeSlider');};RangeSliderField.prototype._initRangeInput=function(){var specificOptions;specificOptions=this.data.type==='apr'?{alias:'decimal',integerDigits:33,suffix:'%'}:{alias:'integer',integerDigits:6,prefix:this.data.type==='currency'?'$ ':''};return this.ui.$rangeInput.inputmask($.extend(specificOptions,{autoGroup:true,groupSeparator:',',allowMinus:false,min:this.data.minValue,max:this.data.maxValue,rightAlign:false}));};RangeSliderField.prototype._addEventListeners=function(){return this.ui.$rangeInput.on('input',this._onInputUpdate).on('blur',this._onInputBlur).on('focus',this._onInputFocus).on('keydown',this._onInputKeyDown);};RangeSliderField.prototype._onInputUpdate=function(){this.$rangeSliderInstance.update({from:this._getValue()});this._dirtySlider();return this._onChange();};RangeSliderField.prototype._onInputFocus=function(){return this._highlightSlider();};RangeSliderField.prototype._onInputKeyDown=function(){if(event.keyCode===this.KEY_CODES.UP){this._setValue(parseInt(this._getValue())+parseInt(this.data.step));return this.$rangeSliderInstance.update({from:this._getValue()});}else if(event.keyCode===this.KEY_CODES.DOWN){this._setValue(parseInt(this._getValue())-parseInt(this.data.step));return this.$rangeSliderInstance.update({from:this._getValue()});}};RangeSliderField.prototype._onInputBlur=function(){this._setValue(this._getValue());return this._unHighlightSlider();};RangeSliderField.prototype._onChange=function(){var base;if(typeof(base=this.callbacks).change==="function"){base.change(this.getCurrentValue({smart:true}),this);}
return this._triggerOnChange();};RangeSliderField.prototype._getValue=function(){return this.ui.$rangeInput.inputmask('unmaskedvalue');};RangeSliderField.prototype._setValue=function(value){this.ui.$rangeInput.val(value);return this._onChange();};RangeSliderField.prototype._highlightSlider=function(){return this.ui.$rangeBlock.addClass('range--focus');};RangeSliderField.prototype._unHighlightSlider=function(){return this.ui.$rangeBlock.removeClass('range--focus');};RangeSliderField.prototype._dirtySlider=function(){return this.ui.$rangeBlock.addClass('range--dirty');};RangeSliderField.prototype._unDirtySlider=function(){return this.ui.$rangeBlock.removeClass('range--dirty');};RangeSliderField.prototype._triggerOnChange=function(){return this.ui.$rangeInput.trigger('rangeInputChange',this.getCurrentValue(),this);};RangeSliderField.prototype._triggerOnMinChange=function(){return this.ui.$rangeInput.trigger('rangeInputMinChange',this.data.minValue,this);};RangeSliderField.prototype._triggerOnMaxChange=function(){return this.ui.$rangeInput.trigger('rangeInputMaxChange',this.data.maxValue,this);};RangeSliderField.prototype._valid=function(){var value;value=this.parseFunction(this._getValue());if(isNaN(value)){return false;}
return(this.data.minValue<=value&&value<=this.data.maxValue);};RangeSliderField.prototype.getCurrentValue=function(options){var stringValue,value;if(options==null){options={};}
_.defaults(options,{smart:false});if(this._valid()){return this.parseFunction(this._getValue());}else if(options.smart){stringValue=this._getValue();value=stringValue===''?0:this.parseFunction(stringValue);if(isNaN(value)){return null;}
if(value>this.data.maxValue){return this.data.maxValue;}
if(value<this.data.minValue){return this.data.minValue;}}else{return null;}};RangeSliderField.prototype.setValue=function(newValue){newValue=this.parseFunction(newValue);if(isNaN(newValue)){return;}
if((this.data.minValue<=newValue&&newValue<=this.data.maxValue)){this._setValue(newValue);this.$rangeSliderInstance.update({from:this._getValue()});return true;}else{return false;}};RangeSliderField.prototype.setMin=function(newMinValue){var oldValue;newMinValue=this.parseFunction(newMinValue);if(isNaN(newMinValue)){return;}
oldValue=this._getValue();if(newMinValue>this.data.maxValue){throw new Error("ERROR :: RangeSliderField class :: min value can't be grater then max value");}
this.data.minValue=newMinValue;this.ui.$rangeInput.inputmask('option',{min:this.data.minValue});this.$rangeSliderInstance.update({min:this.data.minValue});this._triggerOnMinChange();if(oldValue!==this._getValue()){return this._onChange();}};RangeSliderField.prototype.setMax=function(newMaxValue){var oldValue;newMaxValue=this.parseFunction(newMaxValue);if(isNaN(newMaxValue)){return;}
oldValue=this._getValue();if(newMaxValue<this.data.minValue){throw new Error("ERROR :: RangeSliderField class :: max value can't be lower then min value");}
this.data.maxValue=newMaxValue;this.ui.$rangeInput.inputmask('option',{max:this.data.maxValue});this.$rangeSliderInstance.update({max:this.data.maxValue});this._triggerOnMaxChange();if(oldValue!==this._getValue()){return this._onChange();}};RangeSliderField.prototype.isDirty=function(){return this.ui.$rangeBlock.classHasModifier('range','dirty');};RangeSliderField.prototype.on=function(eventName,callback){return this.callbacks[eventName]=callback;};RangeSliderField.prototype.off=function(eventName){return delete this.callbacks[eventName];};return RangeSliderField;})();$(".range:not('.range--with-icons')").each(function(index,element){var type;type=$(element).find('.range__slider').data('mask-type');return new RangeSliderField({DOMElement:element,type:type});});var ENTER_KEYCODE=13;var ESC_KEYCODE=27;var body=$('body');function initMap(){var place={lat:40.746724,lng:-73.983998};var map=new google.maps.Map(document.getElementById('map'),{zoom:16,center:place});var marker=new google.maps.Marker({position:place,icon:'images/svg/map_marker.svg',map:map});}
var iOS=/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream;var iOS2=!!navigator.platform&&/iPad|iPhone|iPod/.test(navigator.platform);function makeSelectCustom(element){element.each(function(){var $this=$(this),numberOfOptions=$(this).children('option').length;$this.addClass('select-hidden');$this.wrap('<div class="select"></div>');$this.after('<div class="select-styled"></div>');var $styledSelect=$this.next('div.select-styled');if($this.data('placeholder')){$styledSelect.text($this.data('placeholder'));}
...
body.on('click','.main-nav__toggle',function(){$(this).toggleClass('main-nav__toggle--opened');$('.main-nav').toggleClass('main-nav--opened');$('.header__contacts').toggleClass('header__contacts--menu-active');$('.header').toggleClass('header--active');});$('.main-nav__list-item--hovered').swipe({tap:function(event,target){$(this).children('.main-nav__list-link').toggleClass('main-nav__list-link--active');}});$(".testimonials__slides-list-wrapper").swipe({swipeLeft:function(event,direction,distance,duration,fingerCount,fingerData){next();},swipeRight:function(event,direction,distance,duration,fingerCount,fingerData){prev();},allowPageScroll:"vertical",threshold:35});function toggleFaq(isAccordion){body.on('click','.faqs__item-header',function(){$target=$(this);$item=$target.closest('.faqs__item');if(isAccordion){if($item.hasClass('faqs__item--active')){$item.removeClass('faqs__item--active');return;}
$('.faqs__item').removeClass('faqs__item--active');$item.addClass('faqs__item--active');}else{$item.toggleClass('faqs__item--active');}});};toggleFaq(true);function toggleGraphs(){body.on('click','.analytics__item-title',function(){$target=$(this);$item=$target.closest('.analytics__item');$('.analytics__item').removeClass('analytics__item--active');$item.addClass('analytics__item--active');});};toggleGraphs();$btnInitOffset=$('.header__contacts-btn-get-started').offset().top;$(window).on('scroll',function(){$('.header__contacts-btn-get-started').addClass('header__contacts-btn-get-started--scroll');if($btnInitOffset>=$('.header__contacts-btn-get-started').offset().top){$('.header__contacts-btn-get-started').removeClass('header__contacts-btn-get-started--scroll');}});function hideHint(){$('.form__fieldset-hint').removeClass('form__fieldset-hint--open');body.off('click',hideHint);}
body.on('click','.form__fieldset-label-hint-btn',function(evt){evt.preventDefault();$(this).siblings('.form__fieldset-hint').addClass('form__fieldset-hint--open');body.on('click',hideHint);});body.on('click','.form__fieldset-hint',function(){$(this).removeClass('form__fieldset-hint--open');});body.on('keydown',function(e){if(e.which===ESC_KEYCODE){$('.form__fieldset-hint').removeClass('form__fieldset-hint--open');}});body.on('click','.form__fieldset-btn-file',function(){$(this).siblings('.form__fieldset-input--file').click();});body.on('keydown','.form__fieldset-btn-file',function(e){if(e.which===ENTER_KEYCODE){$(this).siblings('.form__fieldset-input--file').click();}});body.on('change','.form__fieldset-input--file',function(){var $fileName=$(this).prop('files')[0].name;$(this).siblings('.form__fieldset-input--file-path').val($fileName);});body.on('keydown','.form__fieldset-checkmark',function(e){if(e.which===ENTER_KEYCODE){if(!$(this).siblings('.form__fieldset-checkbox').prop('checked')){$(this).siblings('.form__fieldset-checkbox').prop('checked',true);}else{$(this).siblings('.form__fieldset-checkbox').prop('checked',false);}}});body.on('keydown','.form__fieldset-checkmark-radio',function(e){if(e.which===ENTER_KEYCODE){$(this).siblings('.form__fieldset-radio').click();$('.form__fieldset-label--radio-contractor').removeClass('form__fieldset-label--radio-contractor-active')
$(this).parents('.form__fieldset-label--radio').toggleClass('form__fieldset-label--radio-contractor-active');}});$('.form__fieldset-label--radio-contractor').swipe({tap:function(event,target){$('.form__fieldset-label--radio-contractor').removeClass('form__fieldset-label--radio-contractor-active')
$(this).toggleClass('form__fieldset-label--radio-contractor-active');}});$('.open-dialog').click(function(){$('.popup').show();});$('.open-dialog').keydown(function(e){if(e.which===ENTER_KEYCODE){$('.popup').show();}});$('.popup__close-btn').click(function(){$('.popup').hide();});$(document).keydown(function(e){if(e.which===ESC_KEYCODE){$('.popup').hide();}});$('.popup__btn').click(function(){$('.popup').hide();});$('.popup__btn').keydown(function(e){if(e.which===ENTER_KEYCODE){$('.popup').hide();}});function addNavHover(){$(this).parents('.main-nav__sub-list').addClass('main-nav__sub-list--first-hover');};function removeNavHover(){$(this).parents('.main-nav__sub-list').removeClass('main-nav__sub-list--first-hover');};$subListFirstItems=$('.main-nav__sub-list-item:first-child');$subListFirstItems.on('mouseenter',addNavHover);$subListFirstItems.on('mouseleave',removeNavHover);function toggleAdPopup(e){$(e.target).siblings('.form__fieldset-ad-btn-text-container').toggleClass('form__fieldset-ad-btn-text-container--hidden');}
function closeAdPopup(e){$(e.target).closest('.form__fieldset-ad-btn-text-container').addClass('form__fieldset-ad-btn-text-container--hidden');};if($('.form__fieldset-ad-btn').length>0){body.on('keydown','.form__fieldset-ad-btn',function(e){if(e.which===ENTER_KEYCODE){toggleAdPopup(e);}});$('.form__fieldset-ad-btn').swipe({tap:function(event,target){toggleAdPopup(event);}});body.on('keydown','.form__fieldset-ad-btn-close',function(e){if(e.which===ENTER_KEYCODE){closeAdPopupe(e);}});$('.form__fieldset-ad-btn-close').swipe({tap:function(event,target){closeAdPopup(event);}});}
$('.get-renovation__btn').click(function(e){e.preventDefault();$('.popup').show();});$('.get-renovation__btn').keydown(function(e){if(e.which===ENTER_KEYCODE){e.preventDefault();$('.popup').show();}});$('.popup__close-btn').click(function(){$('.popup').hide();});$(document).keydown(function(e){if(e.which===ESC_KEYCODE){$('.popup').hide();}});$('.popup__btn').click(function(){$('.popup').hide();});$('.popup__btn').keydown(function(e){if(e.which===ENTER_KEYCODE){$('.popup').hide();}});